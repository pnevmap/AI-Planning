...
def run_search(problem, search_function, parameter=None):
    ...
    parameter_name = ""
    if parameter is not None:
        parameter_name = parameter.__name__
    print("{}-{}|{}|{}|{}|{}|{}".format(search_function.__name__ ,parameter_name , ip.succs, ip.goal_tests, ip.states,len(node.solution()),end - start))
    return "\n{}-{}\n".format(search_function.__name__ ,parameter_name ) + show_solution(node, end - start)
    # print()
...
def main(p_choices, s_choices):
    ...
    for pname, p in problems:
        ...
        for sname, s, h in searches:
            ...
            solution_results += run_search(_p, s, _h)
    print(solution_results)

def show_solution(node, elapsed_time):
    result = "Plan length: {}  Time elapsed in seconds: {}\n".format(len(node.solution()), elapsed_time)
